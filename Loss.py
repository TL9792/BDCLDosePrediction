...
def loss(pd_dose,gt_dose, dose_mask, roi, spacing, coarse_dose, gtimg, whole_mask):
    ...
    reconsloss = 0.5*mae_loss(reimg, gtimg, whole_mask)
    consistloss = 0.1*consistloss(pd_dose,dose_mask)
    cdvhloss = 0.1*dvh_loss_c(reimg,gtimg, whole_mask)
    fdvhloss = 0.5*dvh_loss_v(reimg, gtimg, roi, spacing)
    gradloss = 0.5*gradient_loss(pd_dose, gt_dose, dose_mask)
    return (doseloss+reconsloss+consistloss+cdvhloss+fdvhloss+gradloss),doseloss,reconsloss,consistloss,cdvhloss,fdvhloss,gradloss,reimg
